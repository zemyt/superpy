...
def sell_product(product_name, sell_price):
    ...
    field_names = ["id", "product_name", "buy_date", "sell_date", "buy_price", "sell_price", "expiration_date"]
    sold_products = set()
        ...
        with open("bought.csv", "r+") as file:
            ...
            for line in lines:
                ...
                if product_name in line and product_name not in sold_products:
                    data = [bought_data[0], product_name, bought_data[2], sell_date, bought_data[3], sell_price, bought_data[4].strip()]
...
def report_inventory(date):
    ...
    with open("bought.csv", "r") as bought_file, open("sold.csv", "r") as sold_file:
        ...
        for line in bought_lines:
            ...
            if len(inventory_list) == 0 and reference_date >= datetime.strptime(bought_data[2], "%Y-%m-%d").date(): # if inventory_list is empty appends the item
                inventory_list.append([bought_data[1], 1, bought_data[3], bought_data[4]])
                continue

            # checks if the item matches [product_name, buy_price and expiration_date], if True then adds 1 to the existing count
            found = False
            for item in inventory_list:
                if bought_data[1] == item[0] and bought_data[3] == item[2] and bought_data[4] == item[3]:
                    item[1] += 1
                    found = True
                    break  

            # if not found AND the date we are checking is after buy date > append
            if not found and reference_date >= datetime.strptime(bought_data[2], "%Y-%m-%d").date():
                inventory_list.append([bought_data[1], 1, bought_data[3], bought_data[4]])
        ...
        for line in sold_lines:
            ...
            if len(inventory_list) == 0 and reference_date >= datetime.strptime(sold_data[2], "%Y-%m-%d").date() and reference_date < datetime.strptime(sold_data[3], "%Y-%m-%d").date():
            ...
            found = False
            for item in inventory_list:
                if sold_data[1] == item[0] and sold_data[4] == item[2] and sold_data[-1] == item[3] and reference_date >= datetime.strptime(sold_data[2], "%Y-%m-%d").date() and reference_date < datetime.strptime(sold_data[3], "%Y-%m-%d").date():
                    item[1] += 1
                    found = True
                    break
            
            # if not found AND the date we are checking is after buy date > append
            if not found and reference_date >= datetime.strptime(sold_data[2], "%Y-%m-%d").date() and reference_date < datetime.strptime(sold_data[3], "%Y-%m-%d").date():
                inventory_list.append([sold_data[1], 1, sold_data[4], sold_data[-1]])
...
def validate_expiration_date(date):
    try:
        ...
        raise argparse.ArgumentTypeError("Invalid expiration date. Use the format 'YYYY-MM-DD'.")
